...
namespace StarterAssets
{
    ...
    public class ThirdPersonController : MonoBehaviour
    {
        ...
        private float rollBoostMultiplier = 8f;
        private Vector3 targetDirectionCache;
        ...
        private int _interactLayerIndex;
        ...
        private void AssignAnimationIDs()
        {
            ...
            _interactLayerIndex = _animator.GetLayerIndex("Interact");
            _animInteract = Animator.StringToHash("Interacts");
        ...
        private void Move()
        {
            ...
            {
                // Apply roll momentum if rolling
                ...
                targetDirectionCache = targetDirection;
                //targetDirection += _diveImpulse;
               Vector3 rollMove = Vector3.zero;
                if (_rollBoost)
                {
                    rollVelocity = Vector3.Lerp(targetDirection, Vector3.zero, Time.deltaTime * 3f);
                    rollMove = rollVelocity * 0.1f * Time.deltaTime;
                    //UnityEngine.Debug.Log("roll Vector3: " + rollVelocity);
                    //UnityEngine.Debug.Log("roll Horizontal Speed Velocity: " + currentHorizontalSpeed);
                }

                _controller.Move(targetDirection.normalized * (_speed * Time.deltaTime) + rollMove +
                                 new Vector3(0.0f, _verticalVelocity, 0.0f) * Time.deltaTime);
            }
        ...
        private void Dive()
        {
            if (Grounded && _animator.GetBool(_animIDFreeFall) == false)
            {
                ...
                if (_input.roll && !_animator.GetBool(_animIDRoll))
                {
                    ...
                    if (_hasAnimator && (currentHorizontalSpeed > 1))
                    {
                        ...
                        //rollVelocity = transform.forward * 0.2f;
                        _rollBoost = true;
                        //ApplyDiveImpulse();
                    }
                    else
        ...
        {
            rollVelocity = targetDirectionCache;
            //yield return 0; // adjust for your animation length
        ...
        private void Equipped()
        {
            ...
            //UnityEngine.Debug.Log("CURRENT EQUIPPED:" + playerInventory.equippedWeaponKey);
            if (playerInventory != null && playerInventory.equippedWeaponKey != "")
            {
                ...
                if (equippedWeapon == "Model 1897")
                {
                    _animator.SetInteger("WeaponID", 3);
                }
                if (equippedWeapon == "Machete")
                {
                    // UnityEngine.Debug.Log("Setting weapon integar to 7!");
                    _animator.SetInteger("WeaponID", 7);
                }
                return;
            ...
            {
                UnityEngine.Debug.Log("NO WEAPON EQUIPPED");
        ...
        private void Aim()
        {
            if (playerInventory.equippedWeaponKey == "") return;
            ...
            aimCam.Priority = isAiming > 0.05f ? 40 : 10;
            ...
            {
                //_animator.SetBool(_animIDWalkBack, false);
                normalCam.Priority = isAiming < 0.05f ? 20 : 10;
        ...
        public void UpdateTorsoAim()
        {
            ...
            float targetWeight = _input.aim > 0.01f ? 1f : 0f;
        ...
        void TryInteract()
        {
            //UnityEngine.Debug.Log("The interact input is: " + _input.interact);
            if (_input.interact != true)
            {
                //_animator.SetLayerWeight(_interactLayerIndex, 1);
                // 1. Get the current weight of the layer
                float currentWeight = _animator.GetLayerWeight(_interactLayerIndex);

                 // 2. Lerp the current weight towards the target weight
                 // Time.deltaTime * fadeSpeed ensures the fade is frame-rate independent
                currentWeight = Mathf.Lerp(currentWeight, 0, Time.deltaTime * 0.75f);

                // 3. Apply the smoothly changing weight back to the Animator
                _animator.SetLayerWeight(_interactLayerIndex, currentWeight);
                return;
            }   
            //_animator.SetLayerWeight(_interactLayerIndex, 1);
            //_animator.SetTrigger("Interacts");
            if (currentInteractable == null)
            {
                _input.interact = false;
                return;
            }
            UnityEngine.Debug.Log("Interact passed!");
            //Play animation
            _animator.SetLayerWeight(_interactLayerIndex, 1);
            ...
            {
                case InteractType.StoryItem:
                    //playerInventory.AddItem(currentInteractable.itemData);
                    //TriggerPopup(currentInteractable.itemData);
                    currentInteractable.OnPickup();
                    break;

                case InteractType.Weapon:
                    playerInventory.AddWeapon(currentInteractable.weaponData);
                    //DialogueSystem.PlayOneLiner("Got myself some heat!");
                    currentInteractable.OnPickup();
                    break;

                case InteractType.Door:
                    currentInteractable.TriggerDoor();
                    break;

                default:
                    // Generic interact (like switches, triggers, etc)
                    //_animator.SetTrigger("Interact_LArm");
                    currentInteractable.Activate();
                    break;
            }
            _input.interact = false;
            currentInteractable = null; // clear it after use
        }
        ...
        private IEnumerator ResetInteract()
        {
            ...
            _animator.SetLayerWeight(_interactLayerIndex, 0);
        }

        private void JumpAndGravity()
        {
            if (Grounded && _rollBoost == false)
            {
        ...
        void OnTriggerEnter(Collider other)
        {
            ...
            // Auto-pickup logic for ammo & health
            switch (interactable.type)
            {
                case InteractType.Ammo:
                    playerInventory.AddAmmo(interactable.amount);
                    interactable.OnPickup();
                    break;

                case InteractType.Health:
                    playerInventory.AddHealth(interactable.amount);
                    interactable.OnPickup();
                    break;

                default:
                    // For everything else, store as current interactable
        ...
        void OnAnimatorIK(int torsoAimLayerIndex)
        {
            ...
            if (_input.aim > 0.01f && _animator)
            {
                ...
                crosshairHandler.UpdateAimData(aimHit, true);
            }
            else
            {
                crosshairHandler.UpdateAimData(aimHit, false);
            }
            // END NEW 
